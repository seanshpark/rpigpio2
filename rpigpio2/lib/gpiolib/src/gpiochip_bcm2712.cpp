...
unsigned shared_flags;
bcm2712_inst bcm2712_instances[BCM2712_MAX_INSTANCES] = {};

// clang-format off
const char *bcm2712_c0_gpio_alt_names[][BCM2712_FSEL_COUNT - 1] =
{
...
const char *bcm2712_d0_gpio_alt_names[][BCM2712_FSEL_COUNT - 1] =
{
  { "" }, // 0
...
const char *bcm2712_c0_aon_gpio_alt_names[][BCM2712_FSEL_COUNT - 1] =
{
...
const char *bcm2712_d0_aon_gpio_alt_names[][BCM2712_FSEL_COUNT - 1] =
{
...
const int bcm2712_gpio_d0_to_c0[] =
{
  -1, 0, 1, 2, 3, -1, -1, -1, -1, -1,
...
const int bcm2712_gpio_aon_d0_to_c0[] =
{
  0, 1, 2, 3, 4, 5, 6, -1, 7, 8,
...
volatile uint32_t *bcm2712_gpio_base(bcm2712_inst *inst, unsigned gpio,
                                     unsigned int *bit)
{
  ...
  if ((bank >= inst->num_banks) || (gpio >= inst->bank_widths[bank]) || !inst->gpio_base)
    return NULL;
...
volatile uint32_t *bcm2712_pinmux_base(bcm2712_inst *inst, unsigned gpio,
                                       unsigned int *bit)
{
...
volatile uint32_t *bcm2712_pad_base(bcm2712_inst *inst, unsigned gpio,
                                    unsigned int *bit)
{
  unsigned bank, gpio_offset;
...
int bcm2712_gpio_get_level(void *priv, unsigned gpio)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit;
...
void bcm2712_gpio_set_drive(void *priv, unsigned gpio, GPIO_DRIVE_T drv)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit;
  volatile uint32_t *gpio_base = bcm2712_gpio_base(inst, gpio, &bit);
...
GPIO_DRIVE_T bcm2712_gpio_get_drive(void *priv, unsigned gpio)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit;
  volatile uint32_t *gpio_base = bcm2712_gpio_base(inst, gpio, &bit);
...
void bcm2712_gpio_set_dir(void *priv, unsigned gpio, GPIO_DIR_T dir)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit;
  volatile uint32_t *gpio_base = bcm2712_gpio_base(inst, gpio, &bit);
...
GPIO_DIR_T bcm2712_gpio_get_dir(void *priv, unsigned gpio)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit;
  volatile uint32_t *gpio_base = bcm2712_gpio_base(inst, gpio, &bit);
...
GPIO_FSEL_T bcm2712_pinctrl_get_fsel(void *priv, unsigned gpio)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  ...
  if (fsel == 0)
    ...
    return static_cast<GPIO_FSEL_T>(GPIO_FSEL_FUNC1 + (fsel - 1));
...
void bcm2712_pinctrl_set_fsel(void *priv, unsigned gpio, const GPIO_FSEL_T func)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int pinmux_bit;
  volatile uint32_t *pinmux_base = bcm2712_pinmux_base(inst, gpio, &pinmux_bit);
...
GPIO_PULL_T bcm2712_pinctrl_get_pull(void *priv, unsigned gpio)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit;
...
void bcm2712_pinctrl_set_pull(void *priv, unsigned gpio, GPIO_PULL_T pull)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned int bit = 0;
  volatile uint32_t *pad_base = bcm2712_pad_base(inst, gpio, &bit);
...
void *bcm2712_gpio_create_instance(const GPIO_CHIP_T *chip, const char *dtnode)
{
  bcm2712_inst *inst = NULL;
  uint32_t *widths;
  ...
  {
    bcm2712_inst *pinctrl_inst = &bcm2712_instances[i];
...
int bcm2712_gpio_count(void *priv)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
...
void *bcm2712_gpio_probe_instance(void *priv, volatile uint32_t *base)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
...
void *bcm2712_pinctrl_create_instance(const GPIO_CHIP_T *chip, const char *dtnode)
{
  bcm2712_inst *inst = NULL;
  ...
  for (i = 0; i < num_instances; i++)
  {
    bcm2712_inst *gpio_inst = &bcm2712_instances[i];
...
int bcm2712_pinctrl_count(void *priv)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
...
void *bcm2712_pinctrl_probe_instance(void *priv, volatile uint32_t *base)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  unsigned pad_offset;
...
const char *bcm2712_pinctrl_get_fsel_name(void *priv, unsigned gpio, GPIO_FSEL_T fsel)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
...
const char *bcm2712_gpio_get_name(void *priv, unsigned gpio)
{
  bcm2712_inst *inst = (bcm2712_inst *)priv;
  ...
  if ((inst->flags & FLAGS_GPIO) &&
      ((bank >= inst->num_banks) || (gpio_offset >= inst->bank_widths[bank])))
    return NULL;
...
const GPIO_CHIP_INTERFACE_T bcm2712_gpio_interface = {
...
const GPIO_CHIP_INTERFACE_T bcm2712_pinctrl_interface = {
...
using namespace rpigpio2;

DECLARE_GPIO_CHIP(brcmstb, "brcm,brcmstb-gpio", &bcm2712_gpio_interface, 0x40, 0);

DECLARE_GPIO_CHIP(bcm2712, "brcm,bcm2712-pinctrl", &bcm2712_pinctrl_interface, 0x30, 0);
DECLARE_GPIO_CHIP(bcm2712_aon, "brcm,bcm2712-aon-pinctrl", &bcm2712_pinctrl_interface, 0x20,
                  FLAGS_AON);

DECLARE_GPIO_CHIP(bcm2712c0, "brcm,bcm2712c0-pinctrl", &bcm2712_pinctrl_interface, 0x30, FLAGS_C0);
DECLARE_GPIO_CHIP(bcm2712c0_aon, "brcm,bcm2712c0-aon-pinctrl", &bcm2712_pinctrl_interface, 0x20,
                  FLAGS_C0 | FLAGS_AON);

DECLARE_GPIO_CHIP(bcm2712d0, "brcm,bcm2712d0-pinctrl", &bcm2712_pinctrl_interface, 0x20, FLAGS_D0);
DECLARE_GPIO_CHIP(bcm2712d0_aon, "brcm,bcm2712d0-aon-pinctrl", &bcm2712_pinctrl_interface, 0x1c,
                  FLAGS_D0 | FLAGS_AON);
