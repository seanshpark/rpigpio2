...
void *do_read_file(const char *fname, const char *mode, size_t *plen)
{
...
uint64_t dt_translate_addr(const uint32_t *ranges, unsigned ranges_cells, int npa, int nps,
                                  int nca, uint64_t addr)
{
...
void *read_file(const char *fname, size_t *plen) { return do_read_file(fname, "rb", plen); }

void dt_set_path(const char *path) { dtpath = path; }
...
uint32_t dt_read_u32(const char *node, const char *prop) { return dt_read_num(node, prop, 1); }
...
void dt_free(void *value) { free(value); }
