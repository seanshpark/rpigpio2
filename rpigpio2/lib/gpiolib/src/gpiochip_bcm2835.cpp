...
// clang-format off
const char *bcm2835_gpio_alt_names[BCM2835_NUM_GPIOS][BCM2835_ALT_COUNT] =
{
...
const char *bcm2711_gpio_alt_names[BCM2711_NUM_GPIOS][BCM2711_ALT_COUNT] =
{
...
GPIO_FSEL_T bcm2835_gpio_get_fsel(void *priv, unsigned gpio)
{
  ...
  if (gpio < BCM2835_NUM_GPIOS)
  {
    switch ((base[reg] >> lsb) & 7)
    {
      ...
      case 2:
        return GPIO_FSEL_FUNC5;
      case 3:
        return GPIO_FSEL_FUNC4;
      case 4:
        return GPIO_FSEL_FUNC0;
      case 5:
        return GPIO_FSEL_FUNC1;
      case 6:
        return GPIO_FSEL_FUNC2;
      case 7:
        return GPIO_FSEL_FUNC3;
    }
  }
...
void bcm2835_gpio_set_fsel(void *priv, unsigned gpio, const GPIO_FSEL_T func)
{
  volatile uint32_t *base = (volatile uint32_t *)priv;
  /* GPFSEL0-5 with 10 sels per reg, 3 bits per sel (so bits 0:29 used) */
  uint32_t reg = GPFSEL0 + (gpio / 10);
...
GPIO_DIR_T bcm2835_gpio_get_dir(void *priv, unsigned gpio)
{
...
void bcm2835_gpio_set_dir(void *priv, unsigned gpio, GPIO_DIR_T dir)
{
  GPIO_FSEL_T fsel;
  if (dir == DIR_INPUT)
...
int bcm2835_gpio_get_level(void *priv, unsigned gpio)
{
  volatile uint32_t *base = (volatile uint32_t *)priv;
...
void bcm2835_gpio_set_drive(void *priv, unsigned gpio, GPIO_DRIVE_T drv)
{
  volatile uint32_t *base = (volatile uint32_t *)priv;
...
GPIO_PULL_T bcm2835_gpio_get_pull(void *priv, unsigned gpio)
{
  UNUSED(priv);
  UNUSED(gpio);
  /* This is a write-only mechanism */
...
void bcm2835_gpio_set_pull(void *priv, unsigned gpio, GPIO_PULL_T pull)
{
  volatile uint32_t *base = (volatile uint32_t *)priv;
...
const char *bcm2835_gpio_get_name(void *priv, unsigned gpio)
{
  UNUSED(priv);
...
const char *bcm2835_gpio_get_fsel_name(void *priv, unsigned gpio, GPIO_FSEL_T fsel)
{
  UNUSED(priv);
...
GPIO_PULL_T bcm2711_gpio_get_pull(void *priv, unsigned gpio)
{
  volatile uint32_t *base = (volatile uint32_t *)priv;
...
void bcm2711_gpio_set_pull(void *priv, unsigned gpio, GPIO_PULL_T pull)
{
  volatile uint32_t *base = (volatile uint32_t *)priv;
  int reg = GPPUPPDN0 + (gpio / 16);
  int lsb = (gpio % 16) * 2;
...
const char *bcm2711_gpio_get_fsel_name(void *priv, unsigned gpio, GPIO_FSEL_T fsel)
{
  UNUSED(priv);
  const char *name = NULL;
  switch (fsel)
...
void *bcm2835_gpio_create_instance(const GPIO_CHIP_T *chip, const char *dtnode)
{
  UNUSED(dtnode);
  return (void *)chip;
}

int bcm2835_gpio_count(void *priv)
{
  UNUSED(priv);
  return BCM2835_NUM_GPIOS;
}

void *bcm2835_gpio_probe_instance(void *priv, volatile uint32_t *base)
{
  UNUSED(priv);
  return (void *)base;
}

const GPIO_CHIP_INTERFACE_T bcm2835_gpio_interface = {
...
const GPIO_CHIP_INTERFACE_T bcm2711_gpio_interface = {
  .gpio_create_instance = bcm2835_gpio_create_instance,
  .gpio_count = bcm2835_gpio_count,
  .gpio_probe_instance = bcm2835_gpio_probe_instance,
  .gpio_get_fsel = bcm2835_gpio_get_fsel,
...
DECLARE_GPIO_CHIP(bcm2835, "brcm,bcm2835-gpio", &bcm2835_gpio_interface, 0x30000, 0);
DECLARE_GPIO_CHIP(bcm2711, "brcm,bcm2711-gpio", &bcm2711_gpio_interface, 0x30000, 0);
