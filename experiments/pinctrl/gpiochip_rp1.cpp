...
#define rp1_gpio_read32(base, peri_offset, reg_offset) base[(peri_offset + reg_offset) / 4]
...
static void rp1_gpio_ctrl_write(volatile uint32_t *base, int bank, int offset, uint32_t value)
{
...
static void rp1_gpio_pads_write(volatile uint32_t *base, int bank, int offset, uint32_t value)
{
  rp1_gpio_write32(base, gpio_state.pads[bank], RP1_GPIO_PADS_REG_OFFSET(offset), value);
}

static uint32_t rp1_gpio_sys_rio_out_read(volatile uint32_t *base, int bank, int offset)
{
  UNUSED(offset);
  return rp1_gpio_read32(base, gpio_state.sys_rio[bank], RP1_GPIO_SYS_RIO_REG_OUT_OFFSET);
}

static uint32_t rp1_gpio_sys_rio_sync_in_read(volatile uint32_t *base, int bank, int offset)
{
  UNUSED(offset);
  return rp1_gpio_read32(base, gpio_state.sys_rio[bank], RP1_GPIO_SYS_RIO_REG_SYNC_IN_OFFSET);
}

static void rp1_gpio_sys_rio_out_write(volatile uint32_t *base, int bank, int offset,
                                       uint32_t value)
{
  UNUSED(offset);
  rp1_gpio_write32(base, gpio_state.sys_rio[bank], RP1_GPIO_SYS_RIO_REG_OUT_OFFSET, value);
}

static uint32_t rp1_gpio_sys_rio_oe_read(volatile uint32_t *base, int bank)
{
  return rp1_gpio_read32(base, gpio_state.sys_rio[bank], RP1_GPIO_SYS_RIO_REG_OE_OFFSET);
}

static void rp1_gpio_sys_rio_oe_clr(volatile uint32_t *base, int bank, int offset)
{
  rp1_gpio_write32(base, gpio_state.sys_rio[bank], RP1_GPIO_SYS_RIO_REG_OE_OFFSET + RP1_CLR_OFFSET,
                   1U << offset);
}

static void rp1_gpio_sys_rio_oe_set(volatile uint32_t *base, int bank, int offset)
{
  rp1_gpio_write32(base, gpio_state.sys_rio[bank], RP1_GPIO_SYS_RIO_REG_OE_OFFSET + RP1_SET_OFFSET,
                   1U << offset);
}
...
static GPIO_FSEL_T rp1_gpio_get_fsel(void *priv, unsigned gpio)
{
  ...
  rsel = static_cast<RP1_FSEL_T>((reg & RP1_GPIO_CTRL_FSEL_MASK) >> RP1_GPIO_CTRL_FSEL_LSB);
...
static void *rp1_gpio_create_instance(const GPIO_CHIP_T *chip, const char *dtnode)
{
  UNUSED(dtnode);
  return (void *)chip;
}
...
DECLARE_GPIO_CHIP(rp1, "raspberrypi,rp1-gpio", &rp1_gpio_interface, 0x30000, 0);
