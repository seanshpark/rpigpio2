...
#define DECLARE_GPIO_CHIP(name, compatible, iface, size, data) \
  GPIO_CHIP_T name##_chip                                      \
    __attribute__((section("gpiochips"))) = {#name, compatible, iface, size, data}
...
extern const rpigpio::GPIO_CHIP_T __start_gpiochips;
extern const rpigpio::GPIO_CHIP_T __stop_gpiochips;
